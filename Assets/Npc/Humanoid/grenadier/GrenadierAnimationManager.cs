...
/// <summary>
/// Manages all Grenadier NPC animations via Animancer with a 2-layer system.
///
/// LAYER SETUP:
...
///
/// Strukturell identisch zum SoldierAnimationManager.
/// Unterschied: Der Grenadier hat keinen Fire-Loop (Salven-Feuer) wie der Soldier.
/// Er feuert einzelne Granaten, daher gibt es PlayFireShot() aber der Firing-State
/// nutzt trotzdem PlayFiringStance() für die Aim-Pose zwischen den Schüssen.
///
/// SETUP:
...
public class GrenadierAnimationManager : MonoBehaviour, INpcAnimationHandler
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError($"[GrenadierAnimationManager] No Upper Body Mask assigned on {gameObject.name}! " +
    ...
    /// <summary>
    /// Set upper layer to aim-hold loop (firing stance).
    /// Called by Firing.Enter().
    /// </summary>
    public void PlayFiringStance()
    {
        baseLayer.Play(idle);
        SetUpperBaseClip(aimHold);
    }

    /// <summary>
    /// Play fire one-shot on upper layer (instant, no fade).
    /// Forces restart for rapid repeated calls during a salvo.
